    ...
    if (data_size > 0) {
        ...
        if (new_idx == next_loaded) {
            ...
            if (write_byte < new_data.size()) {
                ...
                _unassemble_strs.insert(make_pair(next_loaded, std::move(data_to_store)));
            }
        } else {
    ...
    for (auto iter = _unassemble_strs.begin(); iter != _unassemble_strs.end(); /* nop */) {
        ...
        if (iter->first == next_loaded) {
            ...
            if (write_num < iter->second.size()) {
                ...
                _unassemble_strs.insert(make_pair(next_loaded, std::move(iter->second.substr(write_num))));
