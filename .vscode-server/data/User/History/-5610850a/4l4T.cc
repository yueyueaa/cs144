...
void TCPReceiver::segment_received(const TCPSegment &seg) {
    ...
    if(header.syn == 0) {
        _reassembler.push_substring(seg.payload().copy(), seq_num - 1); //seq-1是没传的值
    }
    else _reassembler.push_substring(seg.payload().copy(), header.fin);  
...
optional<WrappingInt32> TCPReceiver::ackno() const { 
    ...
    if (b_syn) return wrap(_reassembler.first_unassembled() + add, WrappingInt32(s_isn));
    return {}; 
}
